...
## Working rhythm — 커밋 단위 작업 + WORK_LOG

작업은 **의미 있는 커밋 단위**로 쪼개서 진행한다. 한 커밋 = 한 가지 논리적 변경(스캐폴드, 한 모듈 추가, 한 기능 연결 등). 거대한 "전부 다 한 번에" 커밋은 만들지 않는다.

각 커밋을 만든 직후 `docs/WORK_LOG.md`에 그 커밋에 대한 항목을 추가한다:

- **위치:** 시간 정순으로 파일 **맨 아래**에 추가 (역시간순 금지).
- **타이밍:** 커밋 직후 곧바로. 다음 커밋 시작 전에 처리.
- **포함 방식:** WORK_LOG 추가분은 **다음 커밋에 함께 포함**시키거나, WORK_LOG만 별도 커밋해도 무방 — 단, 변경 사항 발생 시점과 기록 시점 사이의 간격을 짧게 유지할 것.
- **항목 형식:** 커밋 해시 + 날짜 + 한 줄 요약 + PRD §14 단계 매핑 + 핵심 변경 + 결정·트레이드오프(있을 때만) + 검증 결과. WORK_LOG.md 상단의 "기록 규칙" 섹션이 단일 진실.

WORK_LOG는 "다음 세션 콜드 스타트용 인계 문서"다. 누군가(또는 미래의 자기 자신) 이 파일만 읽고도 어디까지 됐고 왜 그 결정을 했는지 따라잡을 수 있어야 한다.
